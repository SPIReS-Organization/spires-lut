...
def get_optical_property_data() -> None:
...
class DISORT(object):
    ...
    water coatings as well as lap and snow algae. Adapted code from Niklas Bohn.
    """
    ...
    def __init__(
        ...
        self.wvl_threshold = 5

        # If dir doesn't exist get the data
        if not self.dir_base.exists():
            get_optical_property_data()
        ...
        self.fwhm = np.full_like(self.wvl_op , fill_value=10.0) / 1000.0
        self.wvl_op = self.wvl_op[self.wvl_threshold :]
        self.fwhm = self.fwhm[self.wvl_threshold :]
        nbr_wvl_op = len(self.wvl_op)

        # convert wavelengths to wavenumbers
        self.wvl = self.wvl_op
        self.nbr_wvl = len(self.wvl)
        short_wvl = self.wvl - self.fwhm / 2
        long_wvl = self.wvl + self.fwhm / 2
        ...
        for i in np.arange(0, nbr_lyr, 1):
            ...
            if grain_shape != 1:
                ...
                # Resample
                g_ice_Cg_tmp = wvl_resample(g_ice_Cg_tmp, g_wvl_center, self.wvl_op, np.full_like(self.wvl_op, 0.11))
                gg_ice_F07_tmp = wvl_resample(gg_ice_F07_tmp, g_wvl_center, self.wvl_op, np.full_like(self.wvl_op, 0.11))
        ...
        self.SSA_snw = SSA_snw
        self.MAC_snw = MAC_snw
        self.g_snw = g_snw
        ...
        self.SSA_lap = SSA_lap 
        self.MAC_lap = MAC_lap
        self.g_lap = g_lap
        ...
        pf = TabularLegendreCoefficients(
            ...
            wavelength_grid=self.wvl,
            particle_size_profile=np.array(rho_snw),
            wavelengths=self.wvl,
        )
        ...
        e_dir = e_dir * np.ones_like(self.wvl)
        e_dif = e_dif * np.ones_like(self.wvl)
