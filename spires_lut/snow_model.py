...
def generate_lut(wavelength_file, 
                 output_dir,
                 cpu_cores=1,
                 liquid_water=False, 
                 black_sky=True, 
                 snow_algae=False, 
                 grain_shape="hexagonal plate", # sphere or hexagonal plate for now
                 impurity_type="dust_skiles_size3", # any found in biosnicar ...
                 impurity_max_value=None,
                 output_quantity=["albedo"], # can also return both ['albedo', 'reflectance']
                 relative_azimuth=False,
                 ):
    ...
    dim_map = {
        ...
        "relative_azimuth": np.arange(0, 180.0001, 13) if relative_azimuth else np.array([0.0]),
        "dust_concentration": np.linspace(0, max_lap_value, 5),
        "grain_size": np.arange(30, 1500.0001, 15)
    }
    if snow_algae: dim_map["algae"] = np.arange(0, 6e-5+0.01, 1e-5)
    if liquid_water: dim_map["lwc"] = np.arange(0, 40.0001, 10)
    ...
    for qty in output_quantity:
        root.create_array(
            qty, 
            shape=shape, 
            chunks=(*([1] * len(dim_names)), len(wvl)), 
            dtype='f4'
        )
    ...
    for i in range(0, total_sims, chunk_size):
        ...
        results = Parallel(n_jobs=cpu_cores)(
            delayed(lambda p: DisortSnow(**p).run())(t) for t in tasks
        )

        # Flush to Zarr
        for (vals, res) in zip(chunk, results):
            idx = tuple(np.searchsorted(dim_map[dim_names[k]], vals[k]) for k in range(len(dim_names)))
        ...
        print(f"Simulation progress: {completed}/{total_sims} ({percent:.1f}%)", flush=True)
...
class DisortSnow(object):
    ...
    """
    def __init__(self, sza: float, saa: float, vza: np.array, vaa: np.array, e_dir: np.ndarray,
                 e_dif: np.ndarray, wvl: np.ndarray, fwhm: np.ndarray, nbr_lyr: int, rho_snw: list,
                 dz: list, lap_type: str, mss_cnc_lap: list, mss_cnc_snw_alg: list, rds_snw: list = None,
                 side_length: list = None, depth: list = None, lw_frac: list = None, grain_shape: str = None):
        """Instance of DisortSnow.
        ...
        Args:
            ...
            side_length:     side length of ice crystals, list must have len(nbr_lyr), only if cryo_type = 'ice'
            depth:           depth of ice crystals, list must have len(nbr_lyr), only if cryo_type = 'ice'
        ...
        self.dir_base = Path.home() / ".spires" / "data" / "3.0.zip.unzip" / "biosnicar-py-3.0" / "data" / "OP_data" / "480band"
        ...
        g_wvl_center = g_wvl[1:8]/2 + g_wvl[0:7]/2  #% center point for wavelength band
        ...
        for i in np.arange(0, nbr_lyr, 1):
            ...
            ice_idx = (np.abs(ssps_ice["radii"] - rds_snw[i])).argmin()
            water_idx = (np.abs(ssps_water["radii"] - rds_snw[i])).argmin()
            ...
            g = (w_asm * lw_frac[i] + i_asm * vlm_frac_ice) / (lw_frac[i] + vlm_frac_ice)
            g[g <= 0] = 0.00001
            g[g > 0.99] = 0.99
            g_snw[i, :] = g
            ...
            if grain_shape != "sphere":
                diam_ice = 2.0*rds_snw[i]
                fs_hex0 = 0.788 # hexagonal plate from SNICAR
                fs_hex = 0.788 
                AR_tmp = 2.5 # hexagonal plate from SNICAR
                g_ice_Cg_tmp = g_b0 * (fs_hex0/fs_hex)**g_b1 * diam_ice**g_b2
                gg_ice_F07_tmp = g_F07_p0 + g_F07_p1 * np.log(AR_tmp) + g_F07_p2 * (np.log(AR_tmp))**2; 
                ...
                g_ice_Cg_tmp = wvl_resample(g_ice_Cg_tmp, g_wvl_center, self.wvl_op, np.full_like(self.wvl_op , fill_value=0.1))
                gg_ice_F07_tmp = wvl_resample(gg_ice_F07_tmp, g_wvl_center, self.wvl_op, np.full_like(self.wvl_op, fill_value=0.1))
                g_ice_F07 = gg_ice_F07_tmp + (1.0 - gg_ice_F07_tmp) / SSA_snw[i, :] / 2 # % Eq.2.2 in Fu (2007)
                g_snw[i, :] = g_ice_F07 * g_ice_Cg_tmp  #% Eq.6, He et al. (2017)
        ...
        self.MSSlap[0:nbr_lyr, 1] = np.asarray(mss_cnc_snw_alg) / 917 * 1e-3 # snow algae
        ...
        for i in range(nbr_lyr):
            ...
            for j in range(nbr_lap):
                ...
                g_sum_layer   += self.tau_lap[i, j, :] * self.SSA_lap[j, :] * self.g_lap[j, :]
            ...
            # Effective SSA
            self.SSA[i, :] = (SSA_sum_layer + self.SSA_snw[i, :] * self.tau_snw[i, :]) / self.tau[i, :]

            # Effective asymmetry parameter g
            self.g[i, :] = (g_sum_layer + self.g_snw[i, :] * self.SSA_snw[i, :] * self.tau_snw[i, :]) / (self.tau[i, :] * self.SSA[i, :])
        ...
        pf = TabularLegendreCoefficients(coefficients=lg_coeff, particle_size_grid=np.array(rho_snw),
                                         wavelength_grid=wvl, particle_size_profile=np.array(rho_snw),
                                         wavelengths=wvl)
        ...
        hydro = Hydrostatic(altitude_grid=altitude_grid, pressure_grid=pressure_profile,
                            temperature_grid=temperature_profile, altitude_boundaries=z_grid, particle_mass=mass,
                            gravity=gravity)
        ...
        cp = ComputationalParameters(n_layers=nbr_lyr, n_moments=20, n_streams=16, n_azimuth=1, n_polar=1,
                                     n_user_levels=nbr_lyr + 1)

        mb = ModelBehavior(accuracy=0.0, delta_m_plus=True, do_pseudo_sphere=False, header='', print_variables=None,
                           radius=6371.0)
        ...
        ob = OutputBehavior(incidence_beam_conditions=False, only_fluxes=False, user_angles=True,
        ...
        te = ThermalEmission(thermal_emission=False, bottom_temperature=0.0, top_temperature=0.0, top_emissivity=1.0)
        ...
        oa = OutputArrays(n_polar=cp.n_polar, n_user_levels=cp.n_user_levels, n_azimuth=cp.n_azimuth)
        ...
        sfc = Surface(albedo=0.0, n_streams=cp.n_streams, n_polar=cp.n_polar, n_azimuth=cp.n_azimuth,
                      user_angles=ob.user_angles, only_fluxes=ob.only_fluxes, n_mug=200)
    ...
    def run(self):
        ...
        for ind in range(self.nbr_wvl):
            rfldir, rfldn, flup, dfdt, uavg, uu, albmed, trnmed = disort.disort(
                self.USRANG, self.USRTAU, self.IBCND, self.ONLYFL, self.PRNT, self.PLANK, self.LAMBER, self.DELTAMPLUS,
                self.DO_PSEUDO_SPHERE, self.DTAUC[:, ind], self.SSALB[:, ind], self.PMOM[:, :, ind], self.TEMPER,
                self.WVNMLO, self.WVNMHI, self.UTAU, self.UMU0, self.PHI0, self.UMU, self.PHI, self.FBEAM[ind],
                self.FISOT[ind], self.ALBEDO, self.BTEMP, self.TTEMP, self.TEMIS, self.EARTH_RADIUS, self.H_LYR,
                self.RHOQ, self.RHOU, self.RHO_ACCURATE, self.BEMST, self.EMUST, self.ACCUR, self.HEADER, self.RFLDIR,
                self.RFLDN, self.FLUP, self.DFDT, self.UAVG, self.UU, self.ALBMED, self.TRNMED)
